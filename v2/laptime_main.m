...
idx1 = input("Select track number: ");

if isempty(idx1) || idx1 < 1 || idx1 > numel(allNames)
    error("Invalid track selection.");
end

trackName = allNames{idx1};

% Load or generate the track
if ismember(trackName, savedNames)
    track = load_track(trackName);
else
    switch trackName
        case "triangle"
            track = triangle_track();   
        otherwise
...
% Find saved vehicles
filesV = dir("vehicles/*_veh.mat");
vehNames = erase({filesV.name}, "_veh.mat");

fprintf("Available vehicles:\n");
for i = 1:numel(vehNames)
    fprintf("  %d) %s\n", i, vehNames{i});
end

iv = input("Select vehicle number: ");
vehName = vehNames{iv};

data = load(fullfile("vehicles", vehName + "_veh.mat"), "veh");
veh = data.veh;

fprintf("Loaded vehicle: %s (m = %.1f kg)\n", vehName, veh.m);

data = load(fullfile("vehicles", vehName + "_veh.mat"));
veh = data.veh;
...
vplot(vplot > 100) = NaN;   % hide the "infinite" parts
