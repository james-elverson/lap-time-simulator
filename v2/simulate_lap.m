...
for it = 1:sim.nIters
    ...
    % Forward pass (accel constraint with powertrain + drag)
    for i = 1:(N-1)
        v_i = v(i);

        %  Powertrain: wheel force from engine + gears 
        pt = powertrain(veh, v_i);
        Fx_drive = pt.Fw;   % [N]

        %  Traction limit (can't exceed tire grip) 
        Fx_grip = veh.mu * veh.m * veh.g;  % max longitudinal force [N]
        Fx = min(Fx_drive, Fx_grip);

        %  Resistive forces: drag + rolling 
        Fd  = 0.5 * veh.rho * veh.CdA * v_i^2;    % aero drag
        Frr = veh.Crr * veh.m * veh.g;           % rolling resistance

        % Net forward force
        F_net = Fx - Fd - Frr;
        ax    = F_net / veh.m;   % [m/s^2]

        % Don't let accel go negative in the forward pass
        ax = max(0, ax);

        % Kinematic limit for next speed
        v_next_allow = sqrt(max(0, v_i^2 + 2*ax*ds(i)));
        v(i+1) = min(v(i+1), v_next_allow);

        % enforce segment limit at point i+1 (cornering)
